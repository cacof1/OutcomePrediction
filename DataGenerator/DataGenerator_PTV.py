...
class DataGenerator(torch.utils.data.Dataset):
   ...
   def __getitem__(self, i):
       ...
       if self.inference:
           ...
           censored_label = not(np.int8(self.SubjectList.loc[i, 'xnat_subjectdata_field_map_' + self.config['DATA']['censor_label']]).astype('bool'))
...
def QuerySubjectList(config, session):
   ...
   if 'censor_label' in config['DATA'].keys():
      XML.Add_search_field(
       {"element_name": "xnat:subjectData", "field_ID": "XNAT_SUBJECTDATA_FIELD_MAP=" + str(config['DATA']['censor_label']),
        "sequence": "1", "type": "int"})
   ## Label
   XML.Add_search_field(
...
def LoadClinicalData(config, PatientList):
   ...
   if 'censor_label' in config['DATA'].keys():
       df_trans['xnat_subjectdata_field_map_' + config['DATA']['censor_label']] = PatientList.loc[:, 'xnat_subjectdata_field_map_' + config['DATA']['censor_label']]
